// Regex
const markup_regex = /(\[(.):(.*?):(\2)\]|(\#{1,3}\*?) *(.+?) *(<br>|$))/gim;
const preview_regex = /<(.*?)( class="(.*?)")?>[0-9\. ]*(.*?)<\/(\1)>/gim;
 ...
 */
function markup_to_preview_rec(
  match,
  no_used,
  text_type,
  text_content,
...
function markup_to_preview(text) {
  ...
  subtitle_count = 0;
  single_tags_regex = {
    '<span class="nl"><br></span>': /\[:nl:\]/gim,
    '<span class="np"><br>--<br></span>': /\[:np:\]/gim,
    '<span class="par">&nbsp;&nbsp;</span>': /^\[:par:\]/gim,
    'h$1><span class="par">&nbsp;&nbsp;</span>': /h([1-6])\> *\[:par:\]/gim,
    '<span class="par"><br><br>&nbsp;&nbsp;</span>': /\[:par:\]/gim,
  };
  ...
  content = text
    ...
    .replace(/\>/gim, "&gt;")
    .replace(/\n/gim, "<br>")
    .replace(markup_regex, markup_to_preview_rec)
    .replace(/<br>/gim, " ");
...
function markup_correcter(text) {
  ...
  invalid_chars_errors = [...text.matchAll(/.{0,50}(\\).{0,50}/gim)];

  for (let match of invalid_chars_errors) {
    match[0] = match[0].replace(/(\\)/gim, '<span class="invalid_char">$1</span>').replace(/<br>/gim, "");
    errors.push("<h3>Invalid character</h3><p>... " + match[0] + " ...</p>");
  }

  double_np_errors = [...text.matchAll(/.{0,50}(\[:np:\]){2,}.{0,50}/gim)];

  for (let match of double_np_errors) {
    match[0] = match[0].replace(/(\[:np:\])/gim, '<span class="invalid_char">[:np:]</span>');
    errors.push("<h3>Too many successive new pages</h3><p>... " + match[0] + " ...</p>");
  }

  empty_tags_errors = [...text.matchAll(/.{0,50}\[(i|b): *:\1\].{0,50}/gim)];

  for (let match of empty_tags_errors) {
    match[0] = match[0].replace(/\[(i|b):( *):\1\]/gim, '<span class="invalid_char">[$1:$2:$1]</span>');
...
function converter_to_latex() {
  ...
  let latex = content.replace(/<br>/gim, "");

  return latex
    .replace(preview_regex, preview_to_latex)
    .replace(/\\\&lt;/gim, "<")
    .replace(/\\\&gt;/gim, ">");
}
